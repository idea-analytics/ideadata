.onAttach <- function(...) {
  ...
  packageStartupMessage(cli::rule(left = "Gathering warehouse metadata",
                                  right = paste0("ideadata ", packageVersion("ideadata"))))
  ...
  if(length(unique(warehouse_meta_data$table_name))==0) stop(crayon::red(paste(cli::symbol$circle_cross, "There are 0 tables, which means there's a problem.  Check your VPN or Contact Chris!")))
