...
#' Connect to `ResultsView` View or other tables  in `GPTW` on `REDACTED-SQLSERVER` which provides
...
#' @describeIn get_gptw  `OEQuestions` table in `GPTW` on `REDACTED-SQLSERVER`
...
#' @describeIn get_gptw  `OEResponses` table in `GPTW` on `REDACTED-SQLSERVER`
...
#' @describeIn get_gptw  `Results` table in `GPTW` on `REDACTED-SQLSERVER`
...
#' @describeIn get_gptw  `Sections` table in `GPTW` on `REDACTED-SQLSERVER`
