...
#' Connect to `ResultsView` View or other tables  in `AEES` on `REDACTED-SQLSERVER` which provides
...
#' @describeIn get_aees  `OEQuestions` table in `AEES` on `REDACTED-SQLSERVER`
...
#' @describeIn get_aees  `OEResponses` table in `AEES` on `REDACTED-SQLSERVER`
...
#' @describeIn get_aees  `Questions` table in `AEES` on `REDACTED-SQLSERVER`
...
#' @describeIn get_aees  `Results` table in `AEES` on `REDACTED-SQLSERVER`
...
#' @describeIn get_aees  `Sections` table in `AEES` on `REDACTED-SQLSERVER`
